...
use rumycelium_core::geo::{LAT_E7_MAX, LON_E7_MAX};
use rumycelium_core::{
    EnvSample, GeoPoint, SampleProvenance, SensorModality, Uncertainty,
};
use std::fmt;
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AbiError::WrongLength { expected, actual } => {
                write!(f, "wire record must be exactly {expected} bytes, got {actual}")
            }
...
impl RvEnvSampleV1 {
    ...
    /// half-step; `rumycelium-calibration` widens it with the calibration's
    ...
    pub fn to_env_sample(
        ...
        self.validate()?;
        let modality =
            self.modality().ok_or(AbiError::UnknownModality(self.sensor_type))?;
        ...
        sample.validate().map_err(|e| AbiError::Domain(e.to_string()))?;
        Ok(sample)
    }
}
