    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            CborError::WrongType { expected, found } => {
                write!(f, "wrong cbor major type: expected {expected}, found {found}")
            }
            CborError::WrongLength { expected, actual } => {
                write!(f, "wrong cbor field length: expected {expected}, got {actual}")
            }
...
impl SignedEnvRecordV1 {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, CborError> {
        ...
        let payload: [u8; RV_ENV_SAMPLE_V1_WIRE_LEN] =
            r.read_bytes()?.try_into().map_err(|_| CborError::WrongLength {
                expected: RV_ENV_SAMPLE_V1_WIRE_LEN,
                actual: 0,
            })?;
        let pubkey: [u8; 32] = r.read_bytes()?.try_into().map_err(|_| CborError::WrongLength {
            expected: 32,
            actual: 0,
        })?;
        let signature: [u8; 64] =
