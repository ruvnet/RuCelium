...
/// ingest pipeline's job (`rumycelium-ingest`).
...
mod tests {
    ...
    fn sign_verify_round_trip_through_cbor() {
        let signer = NodeSigner::for_node(b"rumycelium-provision-seed-32-by!", 11);
    ...
    fn wrong_key_rejected() {
        let a = NodeSigner::for_node(b"rumycelium-provision-seed-32-by!", 11);
        let b = NodeSigner::for_node(b"rumycelium-provision-seed-32-by!", 12);
    ...
    fn node_key_derivation_is_deterministic_and_unique() {
        let seed = b"rumycelium-provision-seed-32-by!";
        assert_eq!(
