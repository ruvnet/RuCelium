...
use rumycelium_core::{CalibrationRecord, SensorModality};
...
impl CalibrationStore {
    ...
    pub fn insert(&mut self, record: CalibrationRecord) -> Result<(), CalibrationError> {
        ...
        if self.records.contains_key(&record.calibration_id) {
            return Err(CalibrationError::Core(rumycelium_core::EnvError::Invalid(
                format!(
...
mod tests {
    use super::*;
    use rumycelium_core::calibration::Q16_ONE;
