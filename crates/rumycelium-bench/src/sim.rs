...
use rumycelium_abi::{NodeSigner, RvEnvSampleV1, RV_ENV_FLAG_RETRANSMIT, RV_ENV_SCHEMA_V1};
use rumycelium_core::{GeoPoint, SensorModality};
...
pub const PROVISION_SEED: &[u8; 32] = b"rumycelium-biome-provision-v0.1!";
        ...
        for day in 0..config.days {
            ...
            for tick in 0..ticks_per_day {
                ...
                for (idx, spec) in nodes.iter().enumerate() {
                    ...
                    {
                        let ramp = f64::from(tick - ticks_per_day / 2)
                            / f64::from(ticks_per_day / 2);
                    ...
                    let wire = RvEnvSampleV1 {
                        ...
                        flags: if uplink_down { RV_ENV_FLAG_RETRANSMIT } else { 0 },
