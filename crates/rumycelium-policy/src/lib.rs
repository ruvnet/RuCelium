//! # rumycelium-policy
//!
//! The **governed control path** of the RuMycelium fabric (ADR-264 §9).
...
//!
//! ```compile_fail
//! use rumycelium_policy::{AgentProposal, AuditTrail, GatewayValidator, ProposalKind};
//!
//! let mut gateway = GatewayValidator::new(vec![]);
...
//! let _ = gateway.validate_and_execute(&proposal, 0, |_| String::new(), &mut audit);
//! ```
//!
...
//! ## Determinism
//!
//! No clocks, no RNG: callers pass `now_ns` everywhere, and command signing
//! is deterministic ed25519 (RFC 8032) from a fixed 32-byte seed. Identical
//! runs produce identical signatures, receipts, and receipt hashes.
//!
//! ## Audit
//!
...
//! `"authorized"`, `"signed"`, `"gateway_validated"`, `"executed"`.

#![doc(html_root_url = "https://docs.rs/rumycelium-policy/0.1.0")]
...
pub use pipeline::{
    AuthorityRegistry, AuthorizedProposal, CommandPayload, CommandSigner, EvaluatedProposal,
    ExecutionReceipt, GatewayValidator, PolicyConfig, PolicyEngine, ProposalKindView, SafetyConfig,
    SafetySimulator, SignedCommand, SimulatedProposal,
};
...
pub enum ControlError {
    ...
    /// The command id was already executed (replay protection).
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ControlError::DuplicateCommand(id) => {
                write!(f, "duplicate command {id}: already executed")
            }
...
mod tests {
    use super::*;
    use rumycelium_core::GeoPoint;

    const SEED: &[u8; 32] = b"rumycelium-test-seed-32-bytes-ok";
    ...
    fn run_happy_path() -> (ExecutionReceipt, AuditTrail) {
        ...
        let receipt = gateway
            .validate_and_execute(&cmd, 6_000, |kind| format!("executed {kind:?}"), &mut audit)
            .unwrap();
        (receipt, audit)
    }
    ...
    #[test]
    fn actuator_command_budget_is_enforced() {
        let mut audit = AuditTrail::new();
        let engine = permissive_policy();
    ...
    fn signed_command(
        ...
        let mut registry = AuthorityRegistry::new();
        registry.grant("biome/thames-estuary", "agent/flood", "sluice-7");
        let evaluated = engine.evaluate(actuator_proposal(0.5), 10, audit).unwrap();
    ...
    fn expired_command_is_rejected() {
        ...
        let mut gateway = GatewayValidator::new(vec![signer.public_hex()]);
        let err = gateway
            .validate_and_execute(&cmd, 1_040, |_| String::new(), &mut audit)
            .unwrap_err();
        assert_eq!(
            err,
            ControlError::Expired {
    ...
    fn replaying_a_command_is_duplicate() {
        ...
        let mut gateway = GatewayValidator::new(vec![signer.public_hex()]);
        gateway
            .validate_and_execute(&cmd, 50, |_| "ok".into(), &mut audit)
            .unwrap();
        let err = gateway
            .validate_and_execute(&cmd, 60, |_| "ok".into(), &mut audit)
            .unwrap_err();
    ...
    fn tampered_payload_fails_signature() {
        ...
        let mut gateway = GatewayValidator::new(vec![signer.public_hex()]);
        let err = gateway
            .validate_and_execute(&forged, 50, |_| String::new(), &mut audit)
            .unwrap_err();
    ...
    fn untrusted_key_is_rejected() {
        ...
        let mut gateway = GatewayValidator::new(vec![trusted.public_hex()]);
        let err = gateway
            .validate_and_execute(&cmd, 50, |_| String::new(), &mut audit)
            .unwrap_err();
    ...
    fn signed_command_survives_serde_round_trip() {
        ...
        let mut gateway = GatewayValidator::new(vec![signer.public_hex()]);
        assert!(gateway
            .validate_and_execute(&back, 50, |_| "ok".into(), &mut audit)
            .is_ok());
    }
}
